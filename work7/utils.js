...
function bubbleSort(str){
    ...
    for (let i = 0;i < arr.length -1;i++){
        ...
        for(let j=0;j<arr.length-i-1;j++){
            ...
            sortLog.push([arr.concat(), [j, j + 1]])
            sortLog.push([arr.concat(), [j, j + 1]])
            //比较相邻两个元素，如果左边大，那么就把大的交换到右边
            if(arr[j] > arr[j + 1]){
                //使用解构赋值的方法，把两个元素交换位置
                [arr[j], arr[j + 1]] = [arr[j + 1], arr[j]]
                //把比较之后的数组状态和定位方针保存到sortLog中
                sortLog.push([arr.concat(), [j,j + 1]])
            }
        }
    }
    //把排序最终状态的数组和取消定位方针保存到sortLog中，因为动画的最后状态是不是显示比较数据的
...
function showLog(logValues, showElement){
    let str = ''
    //解构赋值的方式，从排序日志遍历器去取出一条结果，复制给row和pos变量
    let {
        done,
        value: [row,pos]
    } = logValues.next()
